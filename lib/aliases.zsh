...
alias dark="sleep 1s; sh ~/scripts/toggleScreen.sh off && mouseoff"
...
alias dist-update="sudo apt-get dist-update && sudo apt-get dist-upgrade"
alias full-update="sudo apt-get update && sudo apt-get upgrade sudo apt-get dist-update && sudo apt-get dist-upgrade"
alias -g w="which"
alias -g lcm="tail -n 2 ~/.zsh_history | head -n 1 | perl -p -e 's/^[0-9: ]*;//'"
alias -g h100="cat ~/.zsh_history | perl -p -e \"s/^[0-9: ]*;//\" | tail -n 100"
alias -g hall="cat ~/.zsh_history | perl -p -e \"s/^[0-9: ]*;//\""
